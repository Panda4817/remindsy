...
export const getNotificationDate = (week: number, day: number, month: number, year: number) => {
...
export const getNotificationsPermissions = async (value: boolean) => {
	if (value === true) {
...
export const createNotification = async (event: Event) => {
	...
	let nDate = getNotificationDate(event.noticeTime, event.day, event.month, event.startYear);
	const identifier = await Notifications.scheduleNotificationAsync({
		content: {
			title: "Remindsy Update",
			body: `${handleNoticeOutputNotifications(event)} until ${handleOutputNames(event)}'s ${
				event.type
			}! Buy a ${handleCardOrPresOutput(event).toLowerCase()}.`,
			data: { id: event.id },
			sound: "default",
		},
		trigger: {
...
export const deleteNotification = async (id: string) => {
	...
	res.map(async (val) => {
		if (val.content.data.id === id) {
			await Notifications.cancelScheduledNotificationAsync(val.identifier);
		}
		return;
	});
	return;
